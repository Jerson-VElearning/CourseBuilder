...
## See: completion_evaluator.gd, navigation_resolver.gd, tracking/tracking_backend.gd, tracking/debug_tracking_backend.gd, tracking/scorm_backend.gd, resources/quiz_result.gd
...
const _DebugTrackingScript := preload("res://addons/course_builder/tracking/debug_tracking_backend.gd")
const _ScormTrackingScript := preload("res://addons/course_builder/tracking/scorm_backend.gd")
...
func start_course(course_data: IDCourseData) -> void:
	_tracking.end_session()
	course = course_data
	_tracking = _make_tracking()
	_visited.clear()
	_completed.clear()
	_reachable.clear()
	_required.clear()
	...
	if course == null or course.slide_count() == 0:
		...
		return
	_tracking.start_session()
	_scan_quiz_membership()
	_init_quiz_results()
	var first_id := course.get_first_id()
	_reachable[first_id] = true
	var bookmark := _tracking.get_bookmark()
	if not bookmark.is_empty() and course.get_slide(bookmark) != null:
		go_to(bookmark, true)
		return
	if not bookmark.is_empty():
		push_warning("CourseBuilder LMS: unknown resume slide '%s'." % bookmark)
...
func _make_tracking() -> IDTrackingBackend:
	if course == null or course.lms == null or not course.lms.is_tracking_enabled():
		return IDTrackingBackend.new()
	if OS.has_feature("web"):
		return _ScormTrackingScript.new(course.lms, course)
	return _DebugTrackingScript.new(course.lms)
