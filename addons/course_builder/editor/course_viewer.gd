...
## LMS row: tracking preset, summary, Validate, Publish SCORM.
## See: editor/course_viewer_card.gd, editor/slide_link_scanner.gd, editor/lms_validator.gd, editor/scorm_packager.gd, resources/section_data.gd, resources/lms_settings.gd
...
const _LMS_VALIDATOR := preload("res://addons/course_builder/editor/lms_validator.gd")
const _SCORM_PACKAGER := preload("res://addons/course_builder/editor/scorm_packager.gd")
...
@export var lms_preset_option: OptionButton
@export var lms_summary_label: Label
@export var lms_validate_button: Button
@export var lms_publish_button: Button
@export var lms_status_label: Label
...
var _export_dir_dialog: EditorFileDialog
var _zip_dialog: EditorFileDialog
var _pending_export_dir: String = ""
var _pending_remove_index: int = -1
var _pending_section_id: String = ""
var _rebuilding: bool = false
var _lms_syncing: bool = false
...
func _ready() -> void:
	...
	if lms_preset_option != null:
		_fill_lms_preset_option()
		lms_preset_option.item_selected.connect(_on_lms_preset_selected)
	if lms_validate_button != null:
		lms_validate_button.pressed.connect(_on_lms_validate_pressed)
	if lms_publish_button != null:
		lms_publish_button.pressed.connect(_on_lms_publish_pressed)
	if graph_edit != null:
...
func _fill_lms_preset_option() -> void:
	lms_preset_option.clear()
	lms_preset_option.add_item("Off", IDEnums.LmsPreset.OFF)
	lms_preset_option.add_item("Awareness", IDEnums.LmsPreset.AWARENESS)
	lms_preset_option.add_item("Graded exam", IDEnums.LmsPreset.GRADED_EXAM)
	lms_preset_option.add_item("Knowledge checks", IDEnums.LmsPreset.KNOWLEDGE_CHECKS)
	lms_preset_option.add_item("Custom", IDEnums.LmsPreset.CUSTOM)
...
func _setup_dialogs() -> void:
	...
	_export_dir_dialog = EditorFileDialog.new()
	_export_dir_dialog.file_mode = EditorFileDialog.FILE_MODE_OPEN_DIR
	_export_dir_dialog.access = EditorFileDialog.ACCESS_FILESYSTEM
	_export_dir_dialog.title = "Choose Web export folder"
	_export_dir_dialog.dir_selected.connect(_on_export_dir_selected)
	add_child(_export_dir_dialog)

	_zip_dialog = EditorFileDialog.new()
	_zip_dialog.file_mode = EditorFileDialog.FILE_MODE_SAVE_FILE
	_zip_dialog.access = EditorFileDialog.ACCESS_FILESYSTEM
	_zip_dialog.add_filter("*.zip", "SCORM package")
	_zip_dialog.title = "Save SCORM zip"
	_zip_dialog.file_selected.connect(_on_zip_path_selected)
...
func _sync_lms_bar() -> void:
	if lms_preset_option != null and lms_preset_option.item_count == 0:
		_fill_lms_preset_option()
	if _course == null or _course.lms == null:
		_lms_syncing = true
		if lms_preset_option != null:
			lms_preset_option.select(0)
		_lms_syncing = false
		if lms_summary_label != null:
			lms_summary_label.text = "—"
		_set_lms_status("")
		_update_publish_enabled(false)
		return
	var lms := _course.lms
	_lms_syncing = true
	if lms_preset_option != null:
		var idx := int(lms.preset)
		if idx >= 0 and idx < lms_preset_option.item_count:
			lms_preset_option.select(idx)
	_lms_syncing = false
	_refresh_lms_summary()
	_update_publish_enabled(true)


func _refresh_lms_summary() -> void:
	if lms_summary_label == null:
		return
	if _course == null or _course.lms == null:
		lms_summary_label.text = "—"
		return
	lms_summary_label.text = _course.lms.summary_text(_course)


func _on_lms_preset_selected(index: int) -> void:
	if _lms_syncing or _course == null or lms_preset_option == null:
		return
	var lms := _course.ensure_lms()
	var new_preset: IDEnums.LmsPreset = lms_preset_option.get_item_id(index)
	if lms.preset == new_preset:
		return
	lms.preset = new_preset
	lms.apply_to_course(_course)
	_save_course()
	_refresh_lms_summary()
	_update_publish_enabled(true)
	_set_lms_status("")
...
func _on_lms_validate_pressed() -> void:
	if _course == null:
		_set_lms_status("Load a course first.")
		return
	var warnings := _lms_warnings()
	if warnings.is_empty():
		_set_lms_status("LMS: no issues.")
		return
	_show_lms_warnings(warnings)


func _on_lms_publish_pressed() -> void:
	if _course == null:
		_set_lms_status("Load a course first.")
		return
	var lms := _course.ensure_lms()
	if not lms.is_tracking_enabled():
		_set_lms_status("Turn LMS on (not Off) before publishing.")
		return
	var warnings := _lms_warnings()
	if not warnings.is_empty():
		_show_lms_warnings(warnings)
		return
	if _export_dir_dialog == null:
		return
	if not lms.last_export_dir.is_empty() and DirAccess.dir_exists_absolute(lms.last_export_dir):
		_export_dir_dialog.current_dir = lms.last_export_dir
	_export_dir_dialog.popup_centered_ratio(0.6)


func _on_export_dir_selected(dir: String) -> void:
	_pending_export_dir = dir
	if _course != null:
		_course.ensure_lms().last_export_dir = dir
	if _zip_dialog == null:
		return
	var ident := "course"
	if _course != null and _course.lms != null and not _course.lms.identifier.is_empty():
		ident = _course.lms.identifier
	var suggested := dir.path_join("%s.zip" % ident)
	if _course != null and _course.lms != null and not _course.lms.last_zip_path.is_empty():
		suggested = _course.lms.last_zip_path
	_zip_dialog.current_dir = suggested.get_base_dir()
	_zip_dialog.current_file = suggested.get_file()
	_zip_dialog.popup_centered_ratio(0.6)


func _on_zip_path_selected(path: String) -> void:
	if _course == null:
		return
	var zip_path := path
	if zip_path.get_extension().to_lower() != "zip":
		zip_path += ".zip"
	var packager = _SCORM_PACKAGER.new()
	var err: String = packager.package_export(_course, _pending_export_dir, zip_path)
	if not err.is_empty():
		_set_lms_status(err)
		push_warning("CourseBuilder LMS: %s" % err)
		return
	var lms := _course.ensure_lms()
	lms.last_export_dir = _pending_export_dir
	lms.last_zip_path = zip_path
	_save_course()
	_set_lms_status("Packed %s" % zip_path.get_file())


func _lms_warnings() -> PackedStringArray:
	var validator = _LMS_VALIDATOR.new()
	return validator.validate(_course)


func _show_lms_warnings(warnings: PackedStringArray) -> void:
	if warnings.size() == 1:
		_set_lms_status(warnings[0])
	else:
		_set_lms_status("%d LMS warnings. First: %s" % [warnings.size(), warnings[0]])
	for warning in warnings:
		push_warning("CourseBuilder LMS: %s" % warning)


func _update_publish_enabled(course_loaded: bool) -> void:
	if lms_publish_button == null:
		return
	var tracking := course_loaded and _course != null and _course.lms != null and _course.lms.is_tracking_enabled()
	lms_publish_button.disabled = not tracking


func _set_lms_status(text: String) -> void:
	if lms_status_label != null:
		lms_status_label.text = text
...
func _save_course() -> void:
	if _course == null or _course.resource_path.is_empty():
		return
	for quiz_data in _course.quizzes:
		if quiz_data != null and not quiz_data.resource_path.is_empty():
			ResourceSaver.save(quiz_data)
...
func _set_course_controls_enabled(enabled: bool) -> void:
	...
	if lms_preset_option != null:
		lms_preset_option.disabled = not enabled
	if lms_validate_button != null:
		lms_validate_button.disabled = not enabled
	_update_publish_enabled(enabled)
